...
def test_mtreefilev1_get_link_path(mtreefilev1: mtree.MTreeEntryV1, return_value: Path) -> None:
    assert mtreefilev1.get_link_path() == return_value
...
def test_mtree_from_bsdcat(mtreeentryv1_stringio: StringIO) -> None:
    mtree.MTree.from_bsdcat(data=mtreeentryv1_stringio)


def test_mtree_from_bsdcat_invalid_mtree(invalid_mtreeentryv1_stringio: StringIO) -> None:
    with raises(RepoManagementValidationError):
        mtree.MTree.from_bsdcat(data=invalid_mtreeentryv1_stringio)
...
async def test_read_mtree_files() -> None:
    ...
    for package in file_list:
        assert mtree.read_mtree(await extract_from_package_file(await open_package_file(package), ".MTREE"))
