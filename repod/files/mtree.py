...
class MTreeEntry(BaseModel):
    ...
    def get_link_path(self) -> Optional[Path]:
...
class MTree(BaseModel):
    ...
    @classmethod
    def from_bsdcat(self, data: io.StringIO) -> MTree:
        """Create an instance of MTree from an io.StringIO representing the output of bsdcat on an mtree file

        Parameters
        ----------
        data: io.StringIO
            A text stream representing the output of bsdtar called on an mtree file
        ...
        for line in data:
            if line.startswith("/set"):
                ...
                for setting in settings:
                    match setting[0]:
                        case "gid" | "uid":
                            base_settings[setting[0]] = int(setting[1])
                        case "type":
                            base_settings["type_"] = setting[1].strip("\n")
                        case _:
                            base_settings[setting[0]] = setting[1].strip("\n")
                ...
                for setting in settings_list:
                    match setting[0]:
                        case "gid" | "uid" | "size":
                            file_settings[setting[0]] = int(setting[1])
                        case "time":
                            file_settings[setting[0]] = float(setting[1])
                        case "type":
                            # NOTE: do not overload type()
                            file_settings["type_"] = setting[1].strip("\n")
                        case "md5digest" | "sha1digest" | "sha256digest" | "sha384digest" | "sha512digest":
                            ...
                            file_settings[setting[0].strip("digest")] = setting[1].strip("\n")
                        case _:
                            file_settings[setting[0]] = setting[1].strip("\n")
